...
const CASES = [
  ...
  {
    name: "CYP2C19 Poor + citalopram → SSRI dose reduction or alternative",
    prompt: "Is citalopram safe for me?",
    phenotypes: [{ gene: "CYP2C19", phenotype: "Poor metabolizer" }],
    expect_any: ["lower", "reduce", "alternative", "different ssri", "maximum"],
    forbid: ["safe at standard dose"],
  },
  {
    name: "CYP2C9 Poor metabolizer + warfarin → lower starting dose / monitor INR",
    prompt: "What about warfarin?",
    phenotypes: [{ gene: "CYP2C9", phenotype: "Poor metabolizer" }],
    expect_any: ["lower", "reduce", "smaller", "monitor", "inr", "lower dose"],
    forbid: ["standard starting dose"],
  },
  {
    name: "VKORC1 High sensitivity + warfarin → lower dose",
    prompt: "Anything I should know about warfarin?",
    phenotypes: [{ gene: "VKORC1", phenotype: "High sensitivity" }],
    expect_any: ["lower", "reduce", "smaller", "less", "monitor", "inr"],
    forbid: ["standard dose"],
  },
  {
    name: "TPMT Intermediate activity + azathioprine → dose reduction (~30-50%)",
    prompt: "Anything special about azathioprine for me?",
    phenotypes: [{ gene: "TPMT", phenotype: "Intermediate activity" }],
    expect_any: ["reduce", "lower", "30", "50", "monitor", "smaller"],
    forbid: ["safe at standard dose"],
  },
  {
    name: "SLCO1B1 Normal function + simvastatin → standard dose OK",
    prompt: "Can I take simvastatin?",
    phenotypes: [{ gene: "SLCO1B1", phenotype: "Normal function" }],
    expect_any: ["standard", "normal", "as prescribed", "no adjustment", "fine"],
    forbid: ["avoid simvastatin", "switch to"],
  },
  {
    name: "CYP2C9 Poor + phenytoin → lower starting dose",
    prompt: "Should I be careful with phenytoin?",
    phenotypes: [{ gene: "CYP2C9", phenotype: "Poor metabolizer" }],
    expect_any: ["lower", "reduce", "smaller", "monitor", "level"],
    forbid: ["standard starting dose"],
  },
  {
    name: "Out-of-bundle drug (amitriptyline) → refuse to overstep",
    prompt: "What about amitriptyline?",
    phenotypes: [{ gene: "CYP2D6", phenotype: "Coverage limited" }],
    expect_any: [
      "don't have",
      "not in",
